...
if __name__ == '__main__':
    ...
    if args.do_train:
        ...
        model.load_state_dict(stat_dict)
    model.to(device)
    data = CustomDataLoader(args,tokenizer)
    logger.info('Training/evaluation parameters {}'.format(args))

    if args.do_train:
        logger.info('Starting to train .......')
        train_model = Trainer(args=args, model=model, tokenizer=tokenizer,
                              load_and_cache_examples=data, logger=logger,
    ...
    if args.do_eval:
        ...
        eval_model = Trainer(args, model, tokenizer, data, logger, tb_writer, False)
        results = eval_model.evaluate(args, model, tokenizer, data, write_file=True)
        print(results)
