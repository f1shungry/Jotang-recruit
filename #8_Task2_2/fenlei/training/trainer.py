...
def accuracy(args, logits, prog_ids):
    ...
    pred = logits.max(1)[1]
    results = pred.eq(prog_ids).float().mean()
...
class BaseTrainer:
    ...
    def train(self):
        ...
        train_dataloader = \
            self.load_and_cache_examples.data_loader()
        ...
        train_iterator = trange(int(args.num_train_epochs), desc='Epoch', disable=self.global_rank not in [-1, 0])#打进度条
        ...
        for _ in train_iterator:
            ...
            for step, batch in enumerate(self.epoch_iterator):
                self.step = step
                batch = tuple(t.to(args.device) for t in batch)#元组
...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, batch):
        ...
        if args.gradient_accumulation_steps > 1:#可能需要计算几个批次的loss平均值来降低误差
